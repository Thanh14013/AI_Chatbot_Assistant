  ...
  try {
    ...
    openai = {
      embeddings: {
        create: async () => {
          throw new Error(
            'OpenAI client not initialized correctly. Ensure you have the official "openai" npm package installed and OPENAI_API_KEY is set.'
          );
        },
      },
    };
...
export async function generateEmbedding(text) {
  ...
  if (!apiKey) {
    throw new Error(
      "OPENAI_API_KEY not configured. Please set it in your .env file to enable semantic search."
    );
  }
  try {
...
export async function generateAndStoreEmbedding(messageId, content) {
  try {
    ...
    try {
      ...
      // Use the sequelize instance via the imported MessageEmbedding model's sequelize
      await MessageEmbedding.sequelize.query(insertSql, {
        bind: [messageId, vectorString],
        type: MessageEmbedding.sequelize.QueryTypes
          ? MessageEmbedding.sequelize.QueryTypes.INSERT
          : undefined,
      });
      console.log(
        `[embedding] stored (raw insert) for message ${messageId} (len=${embeddingVector.length})`
      );
      return {
        id: null,
      ...
      console.error(
        `[embedding] failed to store for message ${messageId}:`,
        (dbErr === null || dbErr === void 0 ? void 0 : dbErr.stack) ||
          (dbErr === null || dbErr === void 0 ? void 0 : dbErr.message) ||
          dbErr
      );
      return null;
    }
