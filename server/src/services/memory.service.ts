  ...
  try {
    ...
    current.facts = {
      ...
      technical_context: {
        ...
        challenges: mergeTags(
          current.facts.technical_context?.challenges,
          facts.technical_context?.challenges
        ),
        goals: mergeTags(current.facts.technical_context?.goals, facts.technical_context?.goals),
        recent_technologies: mergeTags(
          current.facts.technical_context?.recent_technologies,
          facts.technical_context?.recent_technologies,
...
export async function createUserEvent(
  ...
  event: Omit<CreateMemoryEventInput, "user_id" | "conversation_id">
): Promise<void> {
  try {
    if (!LTM_CONFIG.ENABLED) return;
...
export async function getRelevantEvents(
  ...
  limit: number = 5
): Promise<MemoryEventWithSimilarity[]> {
  try {
    if (!LTM_CONFIG.ENABLED) return [];
    ...
    try {
      ...
      return events as MemoryEventWithSimilarity[];
...
export async function getUserEvents(
  ...
  try {
    if (!LTM_CONFIG.ENABLED) return { events: [], total: 0 };
    ...
    return { events: events as MemoryEvent[], total };
  } catch (error) {
 ...
 */
export async function pruneUserEvents(userId: string): Promise<number> {
  try {
    if (!LTM_CONFIG.ENABLED) return 0;
...
export async function updateConversationSummary(
  ...
  summary: UpdateConversationSummaryInput
): Promise<void> {
  try {
    if (!LTM_CONFIG.ENABLED) return;
    ...
    const count = (messageCount as any[])[0].count;
 ...
 */
function mergeTags(existing?: string[], newTags?: string[], options: MergeOptions = {}): string[] {
  const { maxLength = 50, deduplicate = true, caseSensitive = false } = options;

  if (!newTags || newTags.length === 0) return existing || [];
  if (!existing || existing.length === 0) return newTags.slice(0, maxLength);
 ...
 */
export function isLTMEnabled(): boolean {
 ...
 */
export async function generateChatSuggestions(userId: string): Promise<string[]> {
  try {
    if (!LTM_CONFIG.ENABLED) return [];
...
export async function analyzeAndUpdateMemory(
  ...
  try {
    ...
    if (analysis.facts && Object.keys(analysis.facts).length > 0) {
      const hasFactsToUpdate =
        (analysis.facts.personal && Object.keys(analysis.facts.personal).length > 0) ||
...
export type { UserProfile, UserFacts, MemoryEvent, ConversationSummary, MemoryAnalysis };
