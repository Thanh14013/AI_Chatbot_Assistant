 ...
 */
export const getProjectsByUserId = async (userId: string): Promise<ProjectResponse[]> => {
...
export const deleteProject = async (
  ...
  await Conversation.update(
    { project_id: null, order_in_project: 0 },
 ...
 */
export const getProjectConversations = async (projectId: string, userId: string) => {
  const project = await Project.findByIdActive(projectId);
...
export const moveConversationToProject = async (
  ...
  if (projectId) {
    ...
    conversation.order_in_project = ((maxOrder as number) || 0) + 1;
  } else {
    // Remove from project
...
export const updateConversationOrders = async (
  ...
  // Update orders
  const updates = orders.map(({ conversationId, order }) =>
    Conversation.update(
