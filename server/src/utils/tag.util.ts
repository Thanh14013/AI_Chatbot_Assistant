 ...
 */
export function validateAndNormalizeTags(tags: string[]): TagValidationResult {
  const errors: string[] = [];
  ...
  for (const tag of tags) {
    ...
    if (normalized.length > MAX_TAG_LENGTH) {
      errors.push(
        `Tag too long (max ${MAX_TAG_LENGTH} characters): "${tag}" (normalized: "${normalized}")`
      );
      continue;
    }
 ...
 */
export function areTagsValid(tags: any): boolean {
  if (!Array.isArray(tags)) {
    return false;
  }
